...
export const ACTIVITY_TYPES = [
  ...
  { value: "followup_suggested", label: "Follow-up suggested" },
            ...
            <div className="flex flex-wrap items-center gap-2">
              {suggestedDueAt && (
                <span className="text-gray-500 text-xs">
                  Due: {new Date(suggestedDueAt).toLocaleString(undefined, { dateStyle: "short", timeStyle: "short" })}
                </span>
              )}
              {severity && (
                <span
        ...
        return (
          <div className="space-y-1 text-sm">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {dueAt && (
                <span className="text-gray-500 text-xs">
                  Due: {new Date(dueAt).toLocaleString(undefined, { dateStyle: "short", timeStyle: "short" })}
                </span>
              )}
              {status && (
                <span className="rounded bg-amber-100 px-1.5 py-0.5 text-xs text-amber-800">{status}</span>
              )}
            </div>
          </div>
        );
      ...
      case "task_completed":
        return (
          <p className="text-sm text-gray-700">
            Task marked as <span className="font-medium text-green-700">completed</span>.
          </p>
        );
      case "task_dismissed":
        return (
          <p className="text-sm text-gray-700">
            Task <span className="font-medium text-gray-600">dismissed</span>.
          </p>
        );
      case "task_snoozed": {
        ...
        return (
          <p className="text-sm text-gray-700">
            Task snoozed{until ? ` until ${new Date(until).toLocaleString(undefined, { dateStyle: "short", timeStyle: "short" })}` : ""}.
          </p>
        );
      }
      case "note":
