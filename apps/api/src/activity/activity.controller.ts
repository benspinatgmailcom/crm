...
export class ActivityController {
  ...
  @ApiOperation({
    ...
    description:
      'Creates an activity for an entity. Payload schema depends on type: note {text}, call/meeting {summary?, outcome?, nextStep?}, email {subject?, body?, direction?}, task {title, dueAt?, status?}, ai_summary {text, sources?}, ai_deal_brief {briefMarkdown?, generatedAt?, createdBy?, lookbackDays?, model?, inputTruncationStats?, confidence?}, stage_change {fromStage?, toStage?, reason?, notes?, competitor?, nextSteps?, finalAmount?}.',
  })
